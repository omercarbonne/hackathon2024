...
def prepare_data_train(group):
    ...
    dep_time_sec, arr_time_sec = time_to_seconds(dep_time), time_to_seconds(arr_time)
    total_time_seconds = arr_time_sec - dep_time_sec
    if total_time_seconds < 0: # the trip passes midnight
        full_day = 86400
        total_time_seconds = full_day - dep_time_sec + arr_time_sec
    ...
    res = pd.DataFrame({
        ...
        'line_id': [line_id],
        'departure_time': [dep_time_sec/60],
        'cluster': [cluster],
        'total_distance': [total_dist]})
    return res

def prepare_data_test(group):
    num_of_stations = group['station_index'].max()
    first_station_exist = group['station_index'].min() # should be 1
    dep_time = group.loc[group['station_index'] == first_station_exist, 'arrival_time'].values[0]
    total_passengers = sum(group['passengers_up'])
    ...
    total_dist = calculate_dist(group)
    res = pd.DataFrame({
        'trip_id_unique': group['trip_id_unique'].values[0],
        'num_of_stations': [num_of_stations],
        'total_passengers': [total_passengers],
        'line_id': [line_id],
        'departure_time': [time_to_seconds(dep_time)/60],
        'cluster': [cluster],
        'total_distance': [total_dist]})
    return res
def preprocess_task_2(df_path: str, test: bool) -> str:
    """
    ...
    df_filtered.loc[:,'cluster'] = df_filtered['cluster'].apply(lambda x: clusters_list.index(x))
    # removes all the rows containing NaN values or empty cells
    if test:
        columns_to_keep.remove('arrival_time')
    df_filtered = df_filtered.dropna(subset=columns_to_keep)
    df_filtered.reset_index()

    if test:
        grouped = df_filtered.groupby("trip_id_unique").apply(prepare_data_test).reset_index(drop=True)
    else:
        grouped = df_filtered.groupby("trip_id_unique").apply(prepare_data_train).reset_index(drop=True)
...
if __name__ == '__main__':
    ...
    preprocess_df_path = preprocess_task_2(part_df, False)