...
def fix_door_time(row,valid_row,door_mean_gap_minuets ,arrival_mean_seconds):
    # Column indices
    arrival_time_col_index = 11
...
def fix_row_task1(row,door_mean_gap_seconds ,arrival_mean_seconds):
    #TODO check that each row is not null and fix if it is
    row, valid_row = fix_part(row)
    row, valid_row = fix_arrival_time(row,valid_row,arrival_mean_seconds)
    row, valid_row = fix_door_time(row,valid_row,door_mean_gap_seconds,arrival_mean_seconds) 
    return row, valid_row
...
def preprocess_train_task_2(df_path: str) -> str:
    pass
